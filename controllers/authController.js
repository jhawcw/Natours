...
exports.restrictTo =
  (...roles) =>
  (req, res, next) => {
    // roles is in an array type = ['admin', 'lead-guide']
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  //2) reset password only if token is not expired and user is found, set new password
  if (!user) {
  ...
  //4) Log the user in, send json webtoken to the client
  createSendToken(user, 200, res);
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return new AppError('Password does not match, please try again.', 401);
  }
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  // User.findbyidandupdate will not work as it bypass validation and middleware
